...
/*Resolução prova A - Jessica Laine*/
...
alter table locacao add constraint pk_locacao primary key(CODIGO_LOCACAO);

/*Questão 2:*/

alter table cliente change column rua endereco varchar(70),
drop bairro,
drop cidade,
drop cep,
add unique(cpf),
...
alter table filme modify column duracao decimal not null,
alter estreia set default '2022-01-01';

desc filme;

alter table diretor modify column nome_diretor varchar(50) not null,
add column rua varchar(30), 
add column bairro varchar(30), 
add column cidade varchar(30), 
add column cep decimal(8), 
add column cpf decimal(11), 
add column telefone decimal(15);

alter table genero modify column nome_genero varchar(30) default 'comédia';
desc genero;

alter table locacao add column data_entrega date;
...
/*Questão 4:*/
alter table locacao add constraint fk_locacao_cliente foreign key(codigo_cliente) references cliente(codigo_cliente) on update cascade on delete cascade;
alter table locacao add constraint fk_locacao_filme foreign key(codigo_filme) references filme(codigo_filme) on update cascade on delete cascade;
desc locacao;
